...
export default function Dialog({ milestoneData, onClose, style }: DialogProps) {
  return (
    <div className="overlay bg-iron-950/70 w-full h-full fixed inset-0 flex items-center justify-center">
      <div className="dialog flex-col fixed w-[500px] h-[600px] bg-iron-600 border border-1 border-iron-50 rounded-lg p-6 shadow-lg space-y-6">
        <Button
          type="button"
          icon={<X />}
        ...
        <div className="dialog-header">
          <h3 className={`text-${style.primary} text-2xl font-semibold`}>
            {milestoneData.fr.organisation} ({milestoneData.fr.city})
          </h3>
          <p className={`text-${style.secondary} text-base font-semibold`}>
